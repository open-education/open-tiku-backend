    ...
    if (fetcher.data?.result === true) {
      ...
      if (reqType === StringConst.dictChapterKnowledgeRelationRemove) {
        if (chapterOption.id > 0) {
          httpClient.post<Textbook[]>("/chapter-knowledge/knowledge", {ids: [chapterOption.id]}).then((res) => {
            setKnowledgeLabelList(res);
          }).catch((err) => {
            console.log(err);
          });
        ...
        if (knowledgeOption.id > 0) {
          httpClient.post<Textbook[]>("/chapter-knowledge/chapter", {ids: [knowledgeOption.id]}).then((res) => {
            setChapterLabelList(res);
          }).catch((err) => {
            console.log(err);
          });
