...
import { Alert, Cascader, type CascaderProps, Divider, Drawer, Empty, Form, Input, Layout, Modal, Pagination, Select, theme } from "antd";
...
export default function Index(props: any) {
  ...
  useEffect(() => {
    ...
    httpClient
      .get<Textbook[]>(`/textbook/list/${nodeOption.id}/children`)
      .then((res) => {
  ...
  useEffect(() => {
    ...
    httpClient
      ...
      .catch((err) => {
        setReqErr(<Alert title="Error" description={`读取题目列表出错: ${err.message}`} type="error" showIcon />);
      });
  ...
  const onRejectReasonChange = useCallback((e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
  ...
  const handleModalOk = () => {
    ...
    const req: QuestionEditStatusReq = {
      ...
      status: Number(approveVal),
    };
    httpClient
      .post<boolean>("/edit/status", req)
      .then((res) => {
    ...
    <Layout style={{ padding: "0", minHeight: "100vh" }}>
      ...
      >
        ...
        <div>
          <Form layout={"inline"}>
            <Form.Item label="选择教材或知识点">
              <Cascader style={{ width: "300" }} options={textbookOptions} onChange={onNodeOptionChange} placeholder="请选择选择教材或知识点类别" />
            </Form.Item>

            <Form.Item label="选择题型">
              <Cascader style={{ width: "300" }} options={childNodeOptions} onChange={onChildNodeOptionChange} placeholder="选择题型" />
            </Form.Item>

            <Form.Item label="审核类型">
              <Select value={selectApproveVal} style={{ width: 120 }} onChange={onSelectApproveValChange} options={approveOptions} />
            </Form.Item>
          </Form>
        </div>
        ...
        >
          <Form>
            <Form.Item label="审核类型">
              <Select value={approveVal} style={{ width: 120 }} onChange={onApproveValChange} options={approveOptions} />
            </Form.Item>
