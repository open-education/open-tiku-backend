import { Alert, Button, Cascader, type CascaderProps, Col, Flex, Form, Input, InputNumber, type InputNumberProps, Row, Splitter } from "antd";
                ...
                return (
                  <div className="bg-white" key={item.id}>
                      ...
                      <div>
                        <Flex gap="small" justify="flex-start">
                          ...
                          {(!item.children || item.children.length == 0) && (
                            <Button color="danger" variant="link" onClick={() => onDeleteChapterNameClick(item.id, item.parentId)}>
                              删除
                            </Button>
                          )}
                        </Flex>
                    ...
                    {item.children?.map((row: any, index: number) => {
                      ...
                      return (
                        <div className="ml-1 border-l-2 border-dashed border-blue-950/40 pl-6 mt-1" key={row.id}>
                          {/* 子目录 */}
                            ...
                            <div className="flex items-center">
                              ...
                              <div>
                                <Flex gap="small" justify="flex-start">
                                  ...
                                  <Button color="danger" variant="link" onClick={() => onDeleteChapterNameClick(row.id, item.parentId)}>
                                    删除
                                  </Button>
                                </Flex>
                              </div>
                          ...
                          {item.children?.length - 1 == index && <div className="absolute -left-7 top-[50%] bottom-0 w-2 bg-white"></div>}
                        </div>
                      );
                    })}
                  </div>
              ...
              <Row gutter={[12, 12]}>
                <Col span={24}>
                  <Form layout="horizontal" labelCol={{ span: 5 }} wrapperCol={{ span: 16 }}>
                    ...
                    <Form.Item>
                      <Button color="primary" variant="dashed" onClick={submitEditChapterName}>
                        编辑
                      </Button>
                    </Form.Item>
                  </Form>
