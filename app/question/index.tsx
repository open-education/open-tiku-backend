...
export default function Index(props: any) {
  ...
  // 选择下拉菜单
  const onNodeOptionChange: CascaderProps<TextbookOption>["onChange"] = (
    _,
    selectedOptions,
  ) => {
    if (selectedOptions === undefined) {
      ...
      // 需要去找题型类型和标签
      const nodes =
        ArrayUtil.findPath(textbookOptions, info.id.toString()) ?? [];
      // 题型类型和标签挂载在第三层上
      if (nodes.length < 3) {
        return;
      }
  ...
  const onChildNodeOptionChange: CascaderProps<TextbookOption>["onChange"] = (
    _,
    selectedOptions,
  ) => {
    if (selectedOptions === undefined) {
