    ...
    if (fetcher.data?.result === true) {
      ...
      if (StringConst.dictQuestionsRemove === reqType) {
        if (chapterOption.id > 0) {
          httpClient.get<ChapterAndKnowledgeResp>(`/chapter-knowledge/info/${chapterOption.id}`).then(res => {
            httpClient.get<QuestionCateResp[]>(`/question-cate/list/${res.id}`).then((res) => {
              setChapterLabelList(res);
            }).catch((err) => {
              console.log(err);
        ...
        if (knowledgeOption.id > 0) {
          httpClient.get<ChapterAndKnowledgeResp>(`/chapter-knowledge/info/${knowledgeOption.id}`).then(res => {
            httpClient.get<QuestionCateResp[]>(`/question-cate/list/${res.id}`).then((res) => {
              setKnowledgeLabelList(res);
            }).catch((err) => {
              console.log(err);
